...
class MR_saliency(object):
    ...
    def __MR_superpixel_mean_vector(self,img,labels):
        s = sp.amax(labels)+1
        # recode
        bit = 1 if len(img.shape)<=2 else img.shape[-1]
        vec = sp.zeros((s,bit)).astype(float)
        for i in range(s):
            mask = labels == i
    ...
    def __MR_affinity_matrix(self,img,labels,use_vector=False):     
        # recode 标记使用统计好的向量
    ...
    def __MR_W_D_matrix(self,img,labels):
        '''W 非边缘和相邻的 superPixel距离 为0
        D 对角线为每个superPixel的相邻superPixel 数目
        '''
        s = sp.amax(labels)+1
